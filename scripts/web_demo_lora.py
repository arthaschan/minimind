...
@st.cache_resource
def load_model_tokenizer(model_path, lora_weight='None'):
...
st.session_state.lora_weight = st.sidebar.text_input("LoRA权重路径", value="None", help="输入LoRA权重完整路径，None表示不使用")
...
def main():
    if model_source == "本地模型":
        model, tokenizer = load_model_tokenizer(model_path, st.session_state.lora_weight)
    else:
